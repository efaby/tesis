    ...
    <script>
   		...
   		//valido la cedula q no sea espacio en blanco
			...
			//valida el email
				...
				//if (comprobar_email(document.form1.Email.value)){
			//if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3,4})+$/.test(document.form1.Email.value))){
			...
			//valida que el telefono tenga 9 digitos
      			...
      			alert("Por favor su n&uacute;mero de Tel&eacute;fono debe contener 9 caracteres") 
      			document.form1.Telefono.focus() 
      			return 0; 
   			}
...
function eliminarDato(idcliente){
	...
	if ( eliminar ) {
		...
		ajax.open("GET", "delete_cliente.php?id_cliente="+idcliente);
